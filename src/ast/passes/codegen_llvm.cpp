...
ScopedExpr CodegenLLVM::visit(Identifier &identifier)
{
  ...
  } else if (identifier.ident == "elf_is_pie") {
    return ScopedExpr(
        b_.getInt1(util::is_elf_pie(current_attach_point_->target)));
  } else if (identifier.ident == "elf_is_exe") {
    return ScopedExpr(b_.getInt1(util::is_exe(current_attach_point_->target)));
  } else if (identifier.ident == "elf_ino") {
    return ScopedExpr(
