...
void NamedParamPass::visit(Expression &expr)
{
  ...
  if (call->vargs.size() == 3) {
    ...
    if (!description) {
      call->vargs.at(2).node().addWarning()
          << "Function 'getopt' requires a third string literal parameter"
  ...
  if (auto *default_value = call->vargs.at(1).as<Boolean>()) {
    // boolean
  ...
  if (used_args.contains(arg_name->value)) {
    ...
    if (description &&
        used_args.at(arg_name->value).second != description->value) {
  ...
  used_args[arg_name->value] = std::make_pair(np_default,
                                              description ? description->value
                                                          : "");
  defaults.defaults[arg_name->value] = std::make_pair(
      std::move(np_default), description ? description->value : "");
}
