...
void SemanticAnalyser::visit(Call &call)
{
  ...
  if (call.func == "hist") {
    ...
    if (!call.vargs.at(1).is<Integer>()) {
      call.addError() << "Builtin strnchr requires a non-negative literal";
    }
    if (!bpftrace_.feature_->has_kernel_func(Kfunc::bpf_strnchr)) {
      call.addError() << "kernel not support bpf_strnchr()!";
    }
    call.return_type = CreateInt64();
  } else if (call.func == "strchr") {
    if (!bpftrace_.feature_->has_kernel_func(Kfunc::bpf_strchr)) {
      call.addError() << "kernel not support bpf_strchr()!";
    }
    call.return_type = CreateInt64();
  } else if (call.func == "strchrnul") {
    if (!bpftrace_.feature_->has_kernel_func(Kfunc::bpf_strchrnul)) {
      call.addError() << "kernel not support bpf_strchrnul()!";
    }
    call.return_type = CreateInt64();
  } else if (call.func == "strrchr") {
    if (!bpftrace_.feature_->has_kernel_func(Kfunc::bpf_strrchr)) {
      call.addError() << "kernel not support bpf_strrchr()!";
    }
    call.return_type = CreateInt64();
