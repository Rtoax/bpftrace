    ...
    std::vector<std::string> raw_named_params) const
{
  ...
  for (const auto &[name, default_val] : named_param_info_) {
    ...
    if (it == named_params.end()) {
      switch (*config.type) {
        case GlobalVarConfig::opt_signed:
          named_param_vals[name] = std::get<int64_t>(default_val.value);
          break;
        case GlobalVarConfig::opt_unsigned:
          named_param_vals[name] = std::get<uint64_t>(default_val.value);
          break;
        case GlobalVarConfig::opt_bool:
          named_param_vals[name] = std::get<bool>(default_val.value);
          break;
        case GlobalVarConfig::opt_string:
          named_param_vals[name] = std::get<std::string>(default_val.value);
          break;
      }
      continue;
    }
